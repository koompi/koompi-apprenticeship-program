# Assessment & Certification Guide

## KOOMPI Apprenticeship Program

---

## Overview

The KOOMPI Apprenticeship Program uses a **competency-based assessment system**. This means:

- You advance when you **demonstrate mastery**, not after a fixed time
- Assessment is **continuous**, not just exams
- **Projects and real work** count more than tests
- **Teaching others** is the highest form of proof

---

## Badge System

Every module awards badges based on demonstrated competency:

### Three Badge Levels Per Module

**Foundation Badge**

How to earn:

- Complete all module readings
- Type out all code examples
- Complete self-check exercises
- Pass mentor verification quiz (verbal or written)

Evidence needed:

- Completed exercise files in your project folder
- Mentor sign-off

---

**Builder Badge**

How to earn:

- Complete the module mini-project
- Project must work correctly
- Code must be clean and organized
- Project pushed to GitHub

Evidence needed:

- Working project URL or GitHub repository
- Brief explanation of how you built it
- Mentor code review approval

---

**Master Badge**

How to earn:

- Help at least 1 other apprentice complete this module
- Successfully explain concepts and debug their code
- Your mentee earns their Foundation Badge

Evidence needed:

- Name of apprentice(s) you helped
- Their confirmation that you helped
- Mentor observation or testimonial

---

## Certificate Levels

Certificates are awarded when you complete major milestones:

### Level 1: Digital Citizen Certificate

**Level 1: Digital Citizen**

Requirements:

- Track 00 Complete (all 4 modules, Foundation badges)
- Can type at least 30 WPM in English
- Can navigate Linux terminal with basic commands
- Demonstrates basic English reading comprehension

You have proven:

- Computer literacy
- Basic digital skills
- Foundation for technical learning

### Level 2: Web Developer Apprentice Certificate

**Level 2: Web Developer Apprentice**

Requirements:

- Level 1 Certificate (Digital Citizen)
- HTML Complete (all 7 modules, at least Foundation badges)
- CSS Complete (all 7 modules, at least Foundation badges)
- Portfolio Project: Personal website with multiple pages
- Can communicate with customers in basic English

Portfolio must include:

- Home page with introduction
- About page with your story
- Projects page (even if empty for now)
- Contact page with working links
- Responsive design (works on mobile)

You have proven:

- Can build static websites from scratch
- Understands web fundamentals
- Ready for programming

### Level 3: JavaScript Developer Certificate

**Level 3: JavaScript Developer**

Requirements:

- Level 2 Certificate (Web Developer Apprentice)
- JavaScript Basics Complete (7 modules)
- JavaScript Advanced Complete (5 modules)
- Git & GitHub Complete (4 modules)
- Project: Interactive web application
- All projects on GitHub with proper commits

Project must include:

- User interaction (buttons, forms, etc.)
- DOM manipulation
- At least one API integration
- Clean, commented code
- README documentation

You have proven:

- Can write JavaScript programs
- Understands programming logic
- Uses version control professionally
- Ready for frameworks

### Level 4: Full-Stack Apprentice Certificate

**Level 4: Full-Stack Apprentice**

Requirements:

- Level 3 Certificate (JavaScript Developer)
- React.js Complete (7 modules)
- Next.js & Tailwind Complete (6 modules)
- Project: Full production website or web application
- Professional English communication skills

Project must include:

- Built with React/Next.js
- Styled with Tailwind or custom CSS
- Multiple pages/routes
- Deployed to the internet
- Mobile responsive
- Performance optimized

You have proven:

- Can build modern web applications
- Understands full development workflow
- Ready for professional work

### Graduate: KOOMPI Software Engineer & Entrepreneur

**Graduate: KOOMPI Software Engineer & Entrepreneur**

Requirements:

- Level 4 Certificate (Full-Stack Apprentice)
- Track 03 Complete (Business & Entrepreneurship)
- Demonstrated customer service excellence
- Capstone Project approved by mentors
- Presentation to KOOMPI team

Capstone Project:

- Real-world application or website
- Solves a genuine problem
- Could be for a client, community, or personal startup
- Fully deployed and documented
- Presented and defended

You have proven:

- Technical excellence
- Professional communication
- Business understanding
- Ready for employment or entrepreneurship
- A future leader of Cambodia's tech industry

---

## Skills Assessment Matrix

For each skill area, apprentices are rated on a 4-level scale:

| Level | Name | Description |
|-------|------|-------------|
| 1 | **Novice** | Needs guidance, follows instructions |
| 2 | **Practitioner** | Works independently, may need occasional help |
| 3 | **Proficient** | Works confidently, helps others |
| 4 | **Expert** | Teaches others, innovates, leads |

### Technical Skills Matrix

| Skill Area | Novice | Practitioner | Proficient | Expert |
|------------|--------|--------------|------------|--------|
| HTML | □ | □ | □ | □ |
| CSS | □ | □ | □ | □ |
| JavaScript | □ | □ | □ | □ |
| React | □ | □ | □ | □ |
| Next.js | □ | □ | □ | □ |
| Git/GitHub | □ | □ | □ | □ |
| Linux/Terminal | □ | □ | □ | □ |

### Professional Skills Matrix

| Skill Area | Novice | Practitioner | Proficient | Expert |
|------------|--------|--------------|------------|--------|
| English Communication | □ | □ | □ | □ |
| Customer Service | □ | □ | □ | □ |
| Problem Solving | □ | □ | □ | □ |
| Teamwork | □ | □ | □ | □ |
| Time Management | □ | □ | □ | □ |
| Research Skills | □ | □ | □ | □ |

---

## Assessment Methods

### 1. Self-Check Exercises (Every Lesson)

At the end of each lesson, apprentices complete exercises to verify understanding.

**Format:**

- Coding challenges
- "Can you explain?" questions
- "Build this" mini-tasks

**Example:**

```
Self-Check: HTML Lists

1. Create an unordered list of your 3 favorite foods
2. Create an ordered list of steps to make rice
3. Explain: What is the difference between <ul> and <ol>?
4. Challenge: Create a nested list (a list inside a list)
```

### 2. Module Projects (Every Module)

Each module ends with a practical project that combines all skills learned.

**Evaluation Criteria:**

| Criteria | Weight |
|----------|--------|
| Functionality (Does it work?) | 40% |
| Code Quality (Is it clean?) | 25% |
| Creativity (Did they go beyond?) | 15% |
| Documentation (Can others understand?) | 20% |

### 3. Mentor Verification (Weekly)

Mentors conduct brief check-ins to verify progress:

**Quick Verification Questions (5-10 minutes):**

- Show me your code from this week
- Explain how this part works
- What did you find difficult?
- What will you work on next?

### 4. Peer Teaching (Ongoing)

Apprentices earn Master Badges by teaching others:

**Teaching Assessment:**

- Did the learner understand?
- Was the explanation clear?
- Did the teacher show patience?
- Did both learn something?

### 5. Real Work Performance (Ongoing)

How apprentices perform in real KOOMPI work:

**Observed Behaviors:**

- Customer interaction quality
- Problem-solving approach
- Teamwork and communication
- Initiative and responsibility

---

## Progress Tracking Template

Each apprentice maintains a progress log:

```markdown
# My KOOMPI Apprenticeship Progress

## Personal Information
- Name: _________________
- Start Date: _________________
- Current Level: _________________
- Mentor: _________________

## Certificates Earned
- [ ] Digital Citizen Date: _______
- [ ] Web Developer Apprentice Date: _______
- [ ] JavaScript Developer Date: _______
- [ ] Full-Stack Apprentice Date: _______
- [ ] KOOMPI Graduate Date: _______

## Current Focus
Module: _________________
Started: _________________
Target Completion: _________________

## This Week's Goals
1. _________________
2. _________________
3. _________________

## Completed Modules
| Module | Foundation | Builder | Master | Date |
|--------|------------|---------|--------|------|
| 00.01 | □ | □ | □ | |
| ... | | | | |

## Projects Completed
1. _________________ (GitHub Link)
2. _________________ (GitHub Link)

## Reflection Notes
[Weekly notes on what you learned, challenges, achievements]
```

---

## Assessment Best Practices

### For Mentors

1. **Be encouraging** — Focus on growth, not gaps
2. **Be specific** — "Your loop logic was correct" > "Good job"
3. **Ask questions** — Help them discover answers, don't just tell
4. **Celebrate progress** — Every step forward matters
5. **Document everything** — Keep records of achievements

### For Apprentices

1. **Be honest** — Don't pretend to understand when you don't
2. **Show your work** — Let mentors see your code, even if messy
3. **Ask for feedback** — Seek guidance, not just approval
4. **Track yourself** — Keep your progress log updated
5. **Help others** — Teaching is the best assessment

---

## Assessment Schedule

| Frequency | Activity | Who |
|-----------|----------|-----|
| Daily | Self-check exercises | Apprentice |
| Weekly | Progress check-in | Apprentice + Mentor |
| Per Module | Project submission | Apprentice |
| Per Module | Project review | Mentor |
| Monthly | Skills matrix review | Apprentice + Mentor |
| Per Certificate | Certification evaluation | Mentor Team |
| Graduation | Capstone presentation | Full KOOMPI Team |

---

<div align="center">

**Assessment is not about judgment.**

**It's about knowing where you are and where to go next.**

*Keep learning. Keep building. Keep growing.*

</div>
